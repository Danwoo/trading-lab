...
> **이 서비스**: `bot-agent-service` (:8011) — **봇 만들기 대화**. Claude Agent SDK(`claude-agent-sdk`)를
> 임베드해 사용자의 말을 봇 설정으로 옮긴다. **로컬 배포 모드 전용**이다 — 호스팅에서 셸 권한은
> 테넌트 격리를 무력화하므로(결정 2026-07-28) 호스팅에서는 이 서비스를 **띄우지 않는 것**으로
> 분리한다. 라우트를 골라 빼는 것보다 프로세스를 안 띄우는 편이 빠뜨릴 자리가 없다.
>
> **이 레포에서 유일하게 Anthropic 경로를 쓴다** — 다른 서비스는 OpenAI 호환 `LLM_BASE_URL`/`LLM_MODEL`
> 이고, 여기는 `ANTHROPIC_API_KEY` 하나다 (결정 로그 2026-08-15 ㉮). **사용자의 claude.ai 로그인을
> 태우는 설계는 약관이 막는다** — 조사 결과는 [이슈 #150 코멘트](https://github.com/Danwoo/trading-lab/issues/150#issuecomment-5301722812).
>
> **에이전트에게 무엇을 허용하는가의 정본은 `app/agents/bot_agent.py`** 이고, 같은 내용을
> `tests/test_agent_boundary.py` 가 실행되는 단언으로 한 번 더 잡는다 — 글로만 적으면 코드가
> 조용히 넓어진다. 지금 범위(대화가 폼을 채운다)는 값을 내놓는 일이라 **읽기 도구만** 연다.
> DB·repository 없음 (아래 공통부의 Repository·DB 절은 이 서비스에 해당 없음).
...
- **시각 규약** (#359): 감사·운영 시각 컬럼(`reg_dt`·`mod_dt`·`ingested_at`·`started_dt`·`finished_dt`·`createdAt`·`updatedAt`·`*expiresAt`)은 **`timestamptz`** 다 — 인스턴트를 담고 응답에는 오프셋을 붙인다(`repositories/common/sql_format.DT_ISO_TZ`). **DB 세션 tz 는 UTC 로 코드가 못박고**(`create_sql_engine_from_settings` 의 `-c timezone=UTC`), 기동 때 `core/session_timezone.ensure_session_timezone_utc` 가 fail-closed 로 확인한다. 예외는 **시장 시각 컬럼**(`tn_minute_bar.ts`·`tn_backtest_trade.entry_ts`/`exit_ts`·`tn_nav.nav_dt`) — 시장 현지 벽시계라 naive 로 남고, 그 컬럼을 `CURRENT_TIMESTAMP` 와 쓰거나 비교할 때는 `AT TIME ZONE 'Asia/Seoul'` 을 **명시**한다(안 적으면 뜻이 세션 설정에 딸려 간다).
