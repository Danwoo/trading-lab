...
import { getProxyFailureCode } from "@/utils/common/errors/proxyFailure";
...
/**
 * 서버가 **사유 코드**를 실어 보냈으면 그것으로 우리 표에서 문구를 고른다 (#342 · #423 · #435).
 *
 * 아래 차단이 막는 것은 「서버가 쓴 **문장**」이지 「무슨 일이 났는지」가 아니다. 코드는 닫힌
 * 집합(`EMAIL_FAILURE_CODES`·`STREAM_FAILURE_CODES`·`PROXY_FAILURE_CODES`)이라 서버 원문·내부
 * 호스트·스택이 실릴 자리가 없고, 문구는 클라이언트가 자기 언어 표에서 고르므로 차단의 근거(내부 정보 유출)를
 * 깨지 않는다.
 *
 * 이것이 없으면 손으로 쓴 사용자 안내가 5xx 봉투에 담겨 통째로 버려진다 — 메일 발송 실패
 * 5갈래가 전부 「잠시 후 다시 시도」 하나로 뭉개졌고(#342), 스트리밍 대화도 **다시 시도해도
 * 안 되는** 실패(서비스 부재·키 무효)에 재시도를 시켰다(#423). 프록시가 업스트림에 닿지
 * 못한 것도 같은 자리에서 뭉개져 사용자를 자기 네트워크로 보냈다(#435 B-3).
 */
function messageForReasonCode(error: any, L: typeof ko): string | null {
  ...
  if (streamCode) return L.STREAM_FAILURE_MESSAGES[streamCode];

  const proxyCode = getProxyFailureCode(error);
  return proxyCode ? L.PROXY_FAILURE_MESSAGES[proxyCode] : null;
}
...
export function getApiErrorMessage(error: any): string {
  ...
  const byReasonCode = messageForReasonCode(error, L);
  if (byReasonCode) return byReasonCode;
