...
interface Props {
  ...
  /**
   * 폼 전체에 걸린 실패 — 칸을 짚을 수 없는 것(전략 미선택·다전략)이 여기 온다.
   * **사라지지 않는다.** 토스트는 1.6초 뒤 없어지는데 이 폼은 열 칸이 넘는다 (#453 F1).
   */
    ...
    >
      ...
      <Section title="굴리는 규칙" note="아직 봇을 돌리지는 않습니다 — 저장되는 조건입니다.">
        ...
        <Row
          ...
          help="전략을 여럿 실으면 어떻게 합칠지입니다. 이 화면은 전략을 하나만 실으므로 지금은 쓰이지 않아 잠가 뒀습니다."
        >
          {(control) => (
            <SelectBox
              {...control}
              fieldName="combine_rule"
              // 이 화면은 전략을 하나만 싣는다 — 만져도 아무 일이 안 일어나는 칸을 열어 두지
              // 않는다. 값은 그대로 저장되므로 감추지 않고 **잠근 채 보인다** (#453 F3).
              disabled
