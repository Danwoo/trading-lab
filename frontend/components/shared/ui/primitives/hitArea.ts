...
// **몇 자리인지는 여기 안 적는다** — 주석의 숫자는 드리프트한다(실제로 이 주석이 14, PR 본문이
// 16, 스크립트가 19 를 말하고 있었다 — #289 리뷰). 세는 자리는 하나다:
// `python3 scripts/verify_icon_hit_area.py` 가 매 실행마다 「후보 N건 · 적용 N건 · allowlist N건」
// 을 출력한다(후보 = 적용 + allowlist).
