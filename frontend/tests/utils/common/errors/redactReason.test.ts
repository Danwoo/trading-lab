...
describe("실패 사유는 화면에 URL 을 내보내지 않는다", () => {
  ...
  it("우리가 쓴 한국어 사유는 그대로 남는다", () => {
    // 실제로 백엔드가 내는 문장을 그대로 둔다 — 예시가 낡으면 이 그물이 무엇을 지키는지 흐려진다.
    const ours =
      "toss: 소스가 이 서버의 접근을 막았습니다 (HTTP 403). 먼저 설정 화면에서 키를 확인하세요 — " +
      "값이 틀렸거나 만료됐을 수 있습니다. 키가 확실하다면 발급처 앱 설정에서 이 서버의 IP 를 허용 목록에 등록하세요.";
