...
/**
 * 스키마 모듈을 **런타임에** 훑는다 — `import.meta.glob` 이 아니다.
 *
 * 그 글롭은 죽은 코드 검사(knip)가 정적으로 읽어 `schemas/**` 의 export 를 전부 「쓰임」으로
 * 세게 만든다. 실측으로 죽은 타입 10건이 1건으로 보였다 — 이 그물을 들이는 대가로 다른
 * 그물의 눈을 가리는 셈이라, 파일 목록을 직접 읽고 동적으로 부른다.
 */
const SCHEMAS_DIR = path.resolve(fileURLToPath(new URL(".", import.meta.url)), "../../schemas");
...
const loadModules = async (): Promise<Record<string, Record<string, unknown>>> => {
  const loaded: Record<string, Record<string, unknown>> = {};
  for (const file of schemaFiles(SCHEMAS_DIR).sort()) {
    const key = `schemas/${path.relative(SCHEMAS_DIR, file)}`;
    loaded[key] = (await import(/* @vite-ignore */ pathToFileURL(file).href)) as Record<string, unknown>;
  }
  return loaded;
};
...
const numberFields = (modules: Record<string, Record<string, unknown>>): NumberField[] => {
