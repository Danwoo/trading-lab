...
class Settings(BaseSettings):
    ...
    def _forbid_placeholder_credentials(self) -> "Settings":
        ...
        # **어느 기능이 막히는지 이름을 박지 않는다.** 검사는 설정 전체를 훑는 일반 검사인데
        # 문구만 「파일(SFTP)」로 고정돼 있어서, SFTP 가 목록에서 빠지고 EMAIL 만 남은 상태에서도
        # 파일 기능을 가리켰다 — 남은 키는 이메일인데 파일을 보러 가게 만든다. 남은 키의 이름이
        # 이미 그 자리를 가리키므로 목록까지가 이 경고의 몫이다.
        logging.getLogger("uvicorn.error").warning(
            "자리표시자 자격증명이 남아 있습니다: %s — 실제 값이 들어오기 전까지 그 키를 쓰는 기능은 실패합니다.",
            joined,
        )
        return self
