...
class Capability(BaseModel):
    ...
    **`reason` 은 사용자가 읽는 문장이다.** 어댑터가 쓴 그대로 화면에 나가므로 설계 문서의 결정
    번호(`MD-AD-*`)·구현 단위 이름(「어댑터」)·상류 엔드포인트 이름을 담지 않는다 — 그 말을 아는
    독자는 이 저장소를 가진 사람뿐이다(#319). 근거가 필요하면 코드 주석에 남긴다.
    `scripts/verify_screen_reason_wording.py` 가 실제 사유를 모아 이 규칙을 검사한다."""
