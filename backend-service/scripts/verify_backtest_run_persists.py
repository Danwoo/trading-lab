...
def main() -> int:
    ...
    # **이 그물이 필요로 하는 마이그레이션만 태운다.**
    # 대상 리비전 선택은 `backtest_migration_set.py` 한 벌이다 — `verify_backtest_schema.py` 와
    # 같은 목록을 본다. 스키마를 명시 수식하는 리비전(격리 스키마에서 못 돈다)의 면제도 거기 있다.
    from backtest_migration_set import ordered_migrations_or_exit

    migrations = ordered_migrations_or_exit()
