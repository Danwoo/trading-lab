...
**이사 1회용 도구다.** 리드 결정(2026-08-07)으로 개발 자리 자체가 공개 레포(`trading-lab`)로
옮겨간다 — 이 스크립트가 트리를 한 번 밀고 나면, 그 뒤의 변경은 공개 레포의 보통 커밋·PR 이다.
「매번 하는 릴리스」는 없다. 이 레포는 보관용으로 남는다. 이사 뒤에 이것을 다시 돌리면 목적지
트리를 개발 트리로 통째로 덮으므로(아래 ②), **이사 이후에는 쓰지 않는다.**
