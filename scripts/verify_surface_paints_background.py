...
**못 막는 것**:
  ...
  - **축 ⑤ 는 반대 방향을 못 본다.** 「채움이 그릇 토큰인가」는 잡지만 「그릇이 앱 배경 층인가」
    는 안 본다. 화면이 패널을 안 칠하면 입력의 그릇이 앱 셸까지 내려와, 채움(`--bg-base`)과
    같은 색이 된다 — `/admin` 탭 셸이 `--bg-base` 를 칠하던 판이 그랬고(입력 위로 투명한
    조상 23개) 이 축은 초록이었다. 그 방향은 담는 관계를 따라가야 알 수 있어 정적으로는
    안 잡히고, 브라우저에서 `getComputedStyle` 로 재야 드러난다.
