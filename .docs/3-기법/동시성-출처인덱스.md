...
메시지 큐 consumer·주기 스케줄러·적재 워커 같은 백그라운드 매니저는 **앱 프로세스 안에서** 도는 `asyncio.Task` 다(§3). 따라서 워커를 2개 이상 띄우면 같은 pending 메시지를 두 consumer 가 중복 소비하고, 주간 리포트 메일이 워커 수만큼 중복 발송된다. 매니저가 있는 서비스(backend-service / multi-agent-service)는 의도적으로 단일 프로세스로 운영된다. 흡수로 devactivity 의 APScheduler 스케줄러까지 통합 앱 안으로 들어와, backend-service 는 매니저 셋을 안은 단일 워커다.
...
async def lifespan(app: FastAPI):
    ...
    managers = load_managers()                # message_consumer · scheduler · ingest_worker
...
선언 순서대로 띄우고 종료는 역순이다. → 정본: [backend-service/app/main.py](../../backend-service/app/main.py#L16). 기동 순서는 lifespan 이 아니라 [`modules.py`](../../backend-service/app/modules.py) 의 `MANAGER_MODULES` 목록이 정한다 — 통합 앱은 매니저가 셋(message_consumer · APScheduler 스케줄러 · ingest_worker)이라, 흡수 전 devactivity 가 따로 갖고 있던 `scheduler_manager.start()`/`stop()` 한 쌍도 이 목록의 한 줄로 들어와 같은 루프를 탄다.

### 3.2 매니저 템플릿 — `should_stop` + `create_task` + polling 루프 + graceful cancel

세 매니저가 같은 골격이다. `start()` 가 백그라운드 루프를 `create_task` 로 띄우고, 루프는 `should_stop` 플래그가 설 때까지 동작→`sleep` 주기 대기를 반복한다. `stop()` 은 플래그를 세우고 태스크를 cancel 한 뒤 `CancelledError` 를 삼킨다.

```python
# backend-service/app/managers/message_queue/message_consumer_manager.py:23
async def start(self, message_queue_service=...) -> None:
    ...
    async def loop():
        while not self.should_stop:
            try:
                await run_in_threadpool(           # sync DB I/O 오프로드 (§4)
                    message_queue_service.consume_pending, BATCH_SIZE
                )
                await asyncio.sleep(POLL_INTERVAL)  # 5초 주기 폴링
            except asyncio.CancelledError:
                raise                              # 정상 종료 신호는 전파한다
            except Exception:
                await asyncio.sleep(ERROR_BACKOFF)  # 한 번의 실패로 루프가 죽지 않게
...
같은 골격이 적재 워커([ingest_worker_manager.py](../../backend-service/app/managers/ingest/ingest_worker_manager.py))에도 복제돼 있다 — 주기 상수와 한 번에 하는 일만 다르다. `asyncio.sleep` 은 "다음 주기까지 협력적 양보" 역할이다.

> **`nav.snapshot` 을 발행하는 쪽은 지금 없다.** 종전에는 producer 매니저가 NAV·벤치마크·일간수익률·MDD 를 합성 random-walk 로 만들어 발행했는데, 검산할 수 없는 값이라 걷어냈다(#437, 리드 결정 2026-09-03). 소비 배관(`nav.snapshot` 디스패치·`NavService`·`/nav/history`)은 그대로 두었다 — 진짜 값을 쌓는 쪽이 생기면 그 자리에 꽂는다.
...
backend-service 의 메시지 큐 consumer 는 `tn_message_queue` 의 pending 을 폴링해 topic 별로 처리한다. `nav.snapshot` 을 발행하는 쪽은 지금 없다(#437) — 그 자리가 외부 시세 벤더 스트림이 들어올 확장 지점이다.

```mermaid
flowchart LR
    Prod["(발행자 없음 — 합성 NAV 걷어냄 #437)"] -.-> Pub["MessageQueueService.publish<br/>run_in_threadpool (§4)"]
...
**왜 미루나**: 멤버별 portfolio-mcp-service 활동 조회 + LLM 요약 + SMTP 발송은 수 초~수십 초가 걸린다. 응답을 막지 않고 "발송 시작"만 알린다. background 작업의 감사 컬럼은 요청 신원이 없으므로 `reg_id='system'`.
...
실시간 대시보드(NAV/시세 틱)용 커스텀 훅이었다. **소비자가 0이 돼 삭제됐다**(PR #185) — 실시간 구독은 지금 `lib/terminal/realtimeArbiter` 계열이 맡는다.
...
> devactivity·file 열은 흡수로 backend-service 열에 합쳐졌다 — 두 서비스가 갖고 있던 기법(APScheduler cron · SFTP · Pillow · 업로드 `Semaphore(4)` · `BackgroundTasks` · MCP 소비)이 통합 앱 한 프로세스로 들어왔다. 그래서 이 서비스는 lifespan 매니저 3개를 안은 단일 워커다 (합성 NAV producer 를 걷어낸 뒤 — #437, 리드 결정 2026-09-03). `httpx` 의 file proxy 용도는 in-process 호출로 대체돼 사라졌다.
