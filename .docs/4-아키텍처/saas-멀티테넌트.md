...
사이드바의 업무 메뉴는 "권한 메뉴 ∩ 워크스페이스 메뉴"(§6)다. **가입 흐름이 만드는 개인 워크스페이스에는 기본 업무 메뉴가 함께 부여된다** — 터미널·관심종목(`frontend/constants/protected.ts` 의 `PERSONAL_WORKSPACE_DEFAULT_MENU_IDS`, #251). 대시보드는 화면이 생길 때 이 목록에 더한다(옛 레포 #242 · 이 레포 [#2](https://github.com/Danwoo/trading-lab/issues/2)).
