...
> **⚠ 이 정책은 그것을 만들게 한 사례 둘 중 하나를 못 잡는다 — 리드 판단이 필요하다.**
>
> 초안은 위 두 사례를 「**전부 major**」로 적었다. **사실이 아니다.**
> `cryptography 48.0.1 → 50.0.0` 은 major 지만 `pyjwt 2.12.1 → 2.13.0` 은 **minor** 다
> (major 자리 `2` 가 안 바뀐다). 같은 절이 「9건 중 major 는 `cryptography` 하나뿐」이라고
> 적으면서 동시에 「두 사례가 전부 major」라고 적어 **자기 반증**했다.
>
> 그래서 「major 만 사람 경로」는 **pyjwt 건 같은 minor 의 동작 변경을 그대로 자동 머지시킨다.**
> 리드가 이 정책을 고른 근거가 「그 두 사례가 major 였으니 잡힌다」였으므로, 근거가 무너진 만큼
> 정책을 다시 볼 자리다. 선택지:
>
> - **㉠ 그대로 둔다** — major 만 사람. minor 의 동작 변경은 리뷰 코멘트에만 남고 자동으로 흐른다
> - **㉡ minor 도 사람 경로** — patch 만 자동. 2026-08-07 9건 중 patch 는 2건뿐이라 자동 비율이 크게 준다
> - **㉢ 판정 내용으로 가른다** — 리뷰가 「리드가 읽을 값어치가 있는 사실」을 냈는지로. 버전 자리가
>   아니라 리뷰어의 판단이 기준이 된다. 마커에 그 신호를 실어야 하므로 구현이 는다
>
> **결정 전까지는 §4 의 arm 조건이 major 만 거른다** — 즉 지금은 ㉠ 상태다.
