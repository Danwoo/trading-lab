...
> **자유 배치 부분은 지나갔다 (2026-08-15, `#73` S3).** 아래 O2·O3 이 적은 `GRID_COLUMNS_COUNT`·
> `applyGrid`·`GridCell`·`PanelGrid`·`react-grid-layout` 은 **코드에 없다** — 화면 결정 §20.2 가
> 자유 배치를 걷어냈다. 이 파일은 그때 무엇을 지시했는지의 **기록**이라 본문을 고치지 않는다.
> 지금의 정본은 [`.docs/4-아키텍처/터미널-프론트엔드-구조.md`](../4-아키텍처/터미널-프론트엔드-구조.md) §3.3·§3.4 와
> [`2026-08-09-screen-db-decisions.md`](2026-08-09-screen-db-decisions.md) §20~§21 이다.
