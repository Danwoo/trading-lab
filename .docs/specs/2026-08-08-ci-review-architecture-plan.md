...
- 아래 넷은 초안이 틀렸던 자리다. 현행 코드로 각각 확인하고 그대로 옮겨라:
  ...
  - 커밋 신원이 없어도 **브랜치명 `fix-N-<model>`** 로 저자를 판별한다
    (**2026-08-28 이후 전환기 잔존물** — 브랜치 규약이 `feature/<이슈>-<설명>` 으로 바뀌어
    새 이름에는 벤더 신호가 없다. 신호가 하나도 없으면 `human` 이 아니라 `unknown` 이고
    자동 머지 arm 이 거부된다. 현행은 `scripts/review_route.py` 의 모듈 docstring)
...
- **자기리뷰 차단을 arm 조건으로 승계하라** (조건 ③). `cross-review.yml` 이 「동일-벤더 폴백 +
  ...
  **못 읽으면 arm 하지 않는다** (fail-closed). **2026-08-28 보완**: 브랜치 규약 전환으로
  이름이 벤더를 싣지 않게 되어, 신호가 하나도 없는 경우까지 「못 읽음」에 넣었다 — 종전에는
  그 자리가 `human` 이라 자기리뷰 축에 해당 없음으로 통과했다 (봇 PR 만 면제). **어휘 밖 에이전트형 이메일
  (`unknown_agentish`)은 「사람 저자」가 아니라 「판독 불가」다** — 어휘는 사람이 갱신하므로
  디스패치가 어휘보다 먼저 도는 창이 반드시 생기고, 그 창에서 사람 저자로 접으면 라우팅이
  같은 벤더를 리뷰어로 배정한 채 자기리뷰가 조용히 arm 된다. 작성 티어 판독은 현재 claude
  신원에만 있다 — 그래서 **차단 해제는 리뷰어 벤더가 claude 일 때만** 성립한다(동일-벤더
  kimi·codex 는 늘 사람 경로이고, 혼재 저자에서 claude 티어를 안다는 이유로 kimi 리뷰의
  차단이 풀리지 않는다). **탐지는 `-agent@noreply.local`
  접미 계약에 기댄다** — 접미가 다른 에이전트 신원(`gemini-bot@…`)은 여전히 사람 저자로
  접히므로, 신원 형식을 늘릴 때 `review_route` 의 탐지기부터 늘려야 한다. **마커는 벤더만
  싣고 리뷰어 티어를 싣지 않으므로** 저자 티어를 아는 것이 「다른 티어가 리뷰했다」의 증명은
  아니다 — 폴백이 반대 티어를 고른다는 cross-review 계약을 믿는 구조이고, 실제 대조를 하려면
  마커에 리뷰어 티어를 실어야 한다
...
- **`Refs #N` 도 위험도 출처로 읽는다** (2026-08-09 리드 결정). `closingIssuesReferences` 만
  ...
  - **참조는 산문의 선언에서만 읽는다** — 코드(펜스·중첩 펜스·4칸 들여쓰기 블록·인라인)와
    인용(`>` 및 lazy 연속행) 속 `Refs #N` 은 선언이 아니라 인용이다. **버린 자리의 후보는
    그냥 없애지 말고 위험도를 올리는 쪽으로 쓴다** — 없애면 고위험 참조가 접히고 저위험
    참조만 남아 위험도가 **내려간다**(마크다운 판별은 근사치라 리스트 연속행·펜스 구분선을
    오인한다). 버린 후보가 있으면 low 를 미선언으로 접어야 「좁히는 방향이 fail-closed」가
    참이 된다. **버린 후보는 버린 줄에서 긁어모으지 말고 「본문 전체 탐욕 판독 − 산문 판독」의
    차집합으로 구하라** — 판별의 빈틈이 그대로 구멍이 되지 않게. 그러면 좁힘이 무엇을 잃든
    그 번호가 남아 위험도를 올리는 쪽으로만 작용한다 (회귀 그물이 이 보존을 케이스마다 검사). 본문 어디서든 읽으면 재현 스크립트 인자·예시 한 줄이 위험도
    출처가 되어, 진짜 참조가 없는 PR 이 저위험으로 접힌다 (실측: PR #67 본문에서 뽑힌 4건
    중 진짜 참조는 1건. 레포 전 PR 본문 60건 대조에서 좁힌 뒤 달라지는 것은 2건)
