...
- **자기리뷰 차단을 arm 조건으로 승계하라** (조건 ③). `cross-review.yml` 이 「동일-벤더 폴백 +
  ...
  **못 읽으면 arm 하지 않는다** (fail-closed). **어휘 밖 에이전트형 이메일
  ...
  신원에만 있다 — 동일-벤더 kimi·codex 는 늘 사람 경로다. **탐지는 `-agent@noreply.local`
...
- **`Refs #N` 도 위험도 출처로 읽는다** (2026-08-09 리드 결정). `closingIssuesReferences` 만
  ...
  - **참조는 산문의 선언에서만 읽는다** — 코드 펜스·인용(`>`)·인라인 코드 속 `Refs #N` 은
    선언이 아니라 인용이다. 본문 어디서든 읽으면 재현 스크립트 인자·예시 한 줄이 위험도
