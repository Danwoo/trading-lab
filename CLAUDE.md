...
M2 터미널 골조: 리드가 직접 기동→로그인→「시세」 화면 진입(현행 `/terminal`)→종목 선택→전 패널이 그 종목으로 전환→적재 실행→차트가 적재된 캔들로 그려지고 값이 소스와 일치→소스 없는 패널은 이유와 함께 빔, 까지 확인하면 완료.
...
셸 개편(실험대 홈·46px 레일·372px 사이드 패널·MDI 탭을 `/admin` 으로 격리·자유 배치 철거)은 **M3** 다 — 2026-08-09 셸 결정이 자유 배치를 없앴고, 보드의 격자·곡선은 백테스트·봇 엔진 산출물이라 M2 의 no-go 와 부딪힌다.
