...
M2 터미널 골조: 리드가 직접 기동→로그인→**실험대(홈)가 열림**(봇 0개 첫 진입에서 빈 자리가 무엇이 올 자리인지 말한다)→46px 레일에서 패널을 열면 보드를 안 덮고 372px 로 옆에 붙음→레일의 「시세」에서 종목 선택→전 패널이 그 종목으로 전환→적재 실행→차트가 적재된 캔들로 그려지고 값이 소스와 일치→소스 없는 패널은 이유와 함께 빔→`/admin` 은 MDI 탭으로 열림, 까지 확인하면 완료.
